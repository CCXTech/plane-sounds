...
class senseHatAPI extends Emmiter {
    ...
    async processData(data){
        ...
        const verticalSpeed = this.calculateVerticalSpeed(this.altData, this.prevAltData);
        this.prevAltData = this.altData;

        //Get pitch and roll
        const pitchRad = data.fusionPose.y;
        const rollRad = data.fusionPose.z;
        const pitchDeg = pitchRad * 180 / Math.PI;
        const rollDeg = rollRad * 180 / Math.PI;
        console.log('Pitch: ' + pitchDeg);
        console.log('Roll: ' + rollDeg);
